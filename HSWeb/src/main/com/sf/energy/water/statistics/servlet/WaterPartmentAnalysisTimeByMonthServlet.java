...
public class WaterPartmentAnalysisTimeByMonthServlet extends HttpServlet {
	...
	private void PartmentAnalysisTimeByMonth_ByFenXiang(HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		...
		if(pid == -1||pid == 0)//选了所有部门
		{
			...
			for(int m = 1;m<=12;m++)
			{
					PartmentTimeContrast_month_a = rhi_PartmentTimeContrast_month.getAllschoolFenLeiCountByMonth(year, m);
					PartmentTimeContrast_month_b = rhi_PartmentTimeContrast_month.getAllschoolFenLeiCountByMonth(year-1, m);
