...
public class HWJDCurrentManagerServlet extends HttpServlet
{
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		if ("getHWJDMonitorSWF".equals(method))
		{
			try
			{
				getHWJDMonitorSWF(request, response);
			} catch (Exception e)
			{
				e.printStackTrace();
			}
	 ...
	 * @throws IOException
	 */
	public void getHWJDMonitorSWF(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		String flag = "";
		...
		} else
		{
			...
			try
			{
				String xmlString = apd.getFlashHWJDXML(PumpID);
				response.setContentType("application/x-json");
				PrintWriter out = response.getWriter();
	...
	public void GetMonitorLineInfo(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException, SQLException
	{
		...
		ResultSet rs1 = null;
		if(!MonitorLineList.equals(""))
		{
			
			
			String [] MeterList=MonitorLineList.split(",");
			for(int l=0;l<MeterList.length;l++)
			{
				String Watermeterid=MeterList[l];
				if ((!"".equals(Watermeterid)))
				{
					Calendar c = Calendar.getInstance();
					
					sql = "select valuetime,Zvaluezy,Currentflow,nvl((select max(zgross) from T_monthwater where watermeter_ID="+Watermeterid+" and SELECTYear="+ c.get(Calendar.YEAR)+" and SELECTMonth="+(c.get(Calendar.MONTH)+1)+"),0)Gross  from (select valuetime,Zvaluezy,Currentflow from ZWaterDATAS"+Watermeterid+" order by valuetime )T where ROWNUM=1 ";
					...
					try
					{
						...
						rs = ps.executeQuery();
						String Part_Num = "Meter" + Watermeterid;
						while (rs.next())
						{
							if (write.equals(""))// txtInLine01PowerZYValue
								// divZValueZY
								write = Part_Num + ",ValueTime," + rs.getString("ValueTime") + "|" + Part_Num + ",ZValueZY,"
										+ df.format(rs.getFloat("ZValueZY")) + "|" + Part_Num + ",Currentflow," + df.format(rs.getFloat("Currentflow"))+ "|" + Part_Num + ",Gross," + df.format(rs.getFloat("Gross"));
							else
								write += "|" +Part_Num + ",ValueTime," + rs.getString("ValueTime") + "|" + Part_Num + ",ZValueZY,"
										+ df.format(rs.getFloat("ZValueZY")) + "|" + Part_Num + ",Currentflow," + df.format(rs.getFloat("Currentflow"))+ "|" + Part_Num + ",Gross," + df.format(rs.getFloat("Gross"));
