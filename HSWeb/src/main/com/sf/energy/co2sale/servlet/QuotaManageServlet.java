...
import com.sf.energy.co2sale.impl.CO2QuotaDetlByDevDao;
import com.sf.energy.co2sale.impl.CO2QuotaDetlByPeoDao;
...
public class QuotaManageServlet extends HttpServlet {
			...
			HttpServletResponse response) throws Exception
	{
		...
		if ("getSavingIndexInfoByPartment_ID".equals(method))
			getSavingIndexInfoByPartment_ID(request, response);
		...
		if ("addDeptSavingIndexQuotaInfo".equals(method))
			addDeptSavingIndexQuotaInfo(request, response);
		if ("updateDeptSavingIndexQuotaInfo".equals(method))
			updateDeptSavingIndexQuotaInfo(request, response);
		...
		if ("getWarning".equals(method))
			getWarning(request, response);
	}
			...
			HttpServletResponse response) throws IOException
	{
		...
		try
		{
			...
			if ("success".equals(msg))
			{
				resultInfo = "编辑成功！";
				// 写入日志
				log.writeLog(userLoginName, "部门碳排放定额管理", "更新定额统计信息");
			} else if ("fail".equals(msg))
			{
				resultInfo = "编辑失败！";
				// 写入日志
				log.writeLogErrInfo(userLoginName, "部门碳排放定额管理", "更新定额统计信息",
						"更新失败！！！");
			} else
			{
				resultInfo = msg;
			...
			HttpServletResponse response) throws SQLException, IOException
	{
		...
		String resultInfo = "";
		resultInfo = pfd.getQuotaManageStatisticInfo(strYear, strPartment_ID);
...
*/
			...
			HttpServletResponse response) throws SQLException, IOException
	{
		...
		String strPartment_ID = request.getParameter("strPartment_ID");
		String resultInfo = "";
		if(strPartment_ID.equals("0")){
			resultInfo = pfd.getAllQuotaUseInfo(strYear);
		}else{
	...
	private void queryDeptDevOldInfo(HttpServletRequest request,
			HttpServletResponse response) throws SQLException, IOException
	{
		CO2QuotaDetlByDevDao ddd = new CO2QuotaDetlByDevDao();
		Integer valueID = Integer.parseInt(request.getParameter("valueID"));
	...
	private void addDeptSavingIndexQuotaInfo(HttpServletRequest request,
			HttpServletResponse response) throws IOException
	{
		CO2QuotaManageDao pfd = new CO2QuotaManageDao();
		CO2IndexModel ddm = new CO2IndexModel();
		ddm.setPartment_id(Integer.parseInt(request.getParameter("partid")));
		ddm.setYear(Integer.parseInt(request.getParameter("year")));
		ddm.setStyle_1(Float.parseFloat(request.getParameter("style_1")));
		ddm.setStyle_2(Float.parseFloat(request.getParameter("style_2")));
		ddm.setStyle_3(Float.parseFloat(request.getParameter("style_3")));
		ddm.setStyle_4(Float.parseFloat(request.getParameter("style_4")));
		ddm.setStyle_5(Float.parseFloat(request.getParameter("style_5")));
		ddm.setAdd_1(Float.parseFloat(request.getParameter("add_1")));
		ddm.setAdd_2(Float.parseFloat(request.getParameter("add_2")));
		ddm.setAdd_3(Float.parseFloat(request.getParameter("add_3")));
		ddm.setAdd_4(Float.parseFloat(request.getParameter("add_4")));
		
		String resultInfo = null;
		try
		{
			String msg = pfd.addSavingIndex(ddm);
			if ("success".equals(msg))
			{
				resultInfo = "添加成功！";
			} else if ("fail".equals(msg))
	...
	private void updateDeptSavingIndexQuotaInfo(HttpServletRequest request,
			HttpServletResponse response) throws IOException
	{
		CO2QuotaManageDao pfd = new CO2QuotaManageDao();
		CO2IndexModel ddm = new CO2IndexModel();
		...
		try
		{
			String msg = pfd.updateSavingIndex(ddm);
			if ("success".equals(msg))
			{
				resultInfo = "编辑成功！";
			} else if ("fail".equals(msg))
			...
			HttpServletResponse response) throws SQLException, IOException
	{
		...
		CO2QuotaDetlByPeoDao dpd = new CO2QuotaDetlByPeoDao();
			...
			HttpServletResponse response) throws IOException
	{
		...
		OperationLogInterface log = new OperationLogImplement();		DeptQuotaDetlByPeoModel dpm = new DeptQuotaDetlByPeoModel();
	...
	private void getSavingIndexInfoByPartment_ID(HttpServletRequest request,
			HttpServletResponse response) throws SQLException, IOException
	{
		CO2QuotaManageDao pfd = new CO2QuotaManageDao();
		...
		List<CO2IndexModel> list = pfd.getSavingIndexInfoByPartment_ID(
				strYear, strPartment_ID);
		
		JSONArray json = new JSONArray();
		JSONObject jo = new JSONObject();
		
		for (CO2IndexModel eqmm : list)
		{
			jo = new JSONObject();
			jo.put("partmentID", eqmm.getPartment_id());
			jo.put("partmentName",dp.queryNameById(eqmm.getPartment_id()) );
			jo.put("year", eqmm.getYear());
			jo.put("style_1", eqmm.getStyle_1());
			jo.put("style_2", eqmm.getStyle_2());
			jo.put("style_3", eqmm.getStyle_3());
			jo.put("style_4", eqmm.getStyle_4());
			jo.put("style_5", eqmm.getStyle_5());
			jo.put("add_1", eqmm.getAdd_1());
			jo.put("add_2", eqmm.getAdd_2());
			jo.put("add_3", eqmm.getAdd_3());
			jo.put("add_4", eqmm.getAdd_4());
			json.add(jo);
		}
	...
	private void getWarning(HttpServletRequest request,
			HttpServletResponse response) throws SQLException, IOException
	{
		...
		ArrayList<EnergyQuotaManagerModel> list = pfd.getAllWarningInfo(year,userID);
		JSONArray json = new JSONArray();
		JSONObject jo = new JSONObject();
		...
		for (EnergyQuotaManagerModel eqmm : list)
		{
			...
			jo.put("addPerMonthQuota", eqmm.getAddPerMonthQuota());
			jo.put("ZMoney", eqmm.getZMoney());
			jo.put("buyQuota", eqmm.getBuyQuota());
			jo.put("saleQuota",eqmm.getSaleQuota());
			jo.put("saveData", Float.parseFloat(eqmm.getTotalQuota())*Float.parseFloat(eqmm.getAdd_1())/100);
			jo.put("remainQuota", eqmm.getRemainQuota());
			jo.put("remainRate", eqmm.getZMoney());
			json.add(jo);
		}
