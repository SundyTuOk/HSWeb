...
public class AmStandByModelServerlet extends HttpServlet
{
			...
			throws SQLException, IOException, ParseException
	{
		...
		if ("getAmMeterModelBetween".equals(method))
			getAmMeterModelBetween(req, resp);
	...
	{
		AmStandByModelDao model = new AmStandByModelDao();
			...
			HttpServletResponse response) throws IOException, SQLException
	{
		...
		List<MeterInfoDao> list = info.paginate(thePageCount, thePageIndex,area_id,arch_id,tableName,order);
	 ...
	 * @throws IOException
	 */
	private void getAmMeterModelBetween(HttpServletRequest req,
...
//			end.setDate(day+1);
		...
		String meterID = req.getParameter("AmMeterID").trim();
		int AmMeterID = Integer.parseInt(meterID);
		
		
		ElecReportHelperImpl rhi_AmStandByModel = new ElecReportHelperImpl();
		List<AmMeterMataData> dataList = rhi_AmStandByModel.getAmMeterBetween(AmMeterID, start, start);
