...
public class EncoderOfClassroom
{
	/**
	 * 字符串补齐，不足位用"0"补齐
	 * 
	...
	public static int GeneralFrame(byte[] sendbuffer, String commandcode,
			String jcqstartaddress, String deviceaddress, String para,
			int startindex)
	{
		sendbuffer[startindex] = (byte) (Integer.parseInt(deviceaddress));// 地址域

		sendbuffer[1 + startindex] = strToByte(commandcode); // 功能码

		sendbuffer[2 + startindex] = strToByte(jcqstartaddress.substring(0, 2));// 寄存器起始地址
		sendbuffer[3 + startindex] = strToByte(jcqstartaddress.substring(2, 4));

		int len = 0;
		if ("03".equals(commandcode))// 读保持寄存器
		{
			len = Parse03(sendbuffer, para, startindex);
		} else if (commandcode == "10") // 写多个寄存器
		{
			len = Parse10(sendbuffer, jcqstartaddress, para, startindex);
		}

		return len;
	}

	private static int Parse03(byte[] sendbuffer, String para, int startindex)
	{
		int num = Integer.parseInt(para);
		sendbuffer[4 + startindex] = (byte) (num / 256); // 寄存器数量
		sendbuffer[5 + startindex] = (byte) (num % 256);
	...
	private static int Parse10(byte[] sendbuffer, String jcqstartaddress,
			String para, int startindex)
	{
		int len = 0;

		switch (jcqstartaddress)
		{
		case "0404":// 应急
			sendbuffer[4 + startindex] = 0x00; // 寄存器数量
			sendbuffer[5 + startindex] = 0x01;
			sendbuffer[6 + startindex] = 0x02; // 字节数
			sendbuffer[7 + startindex] = 0x00; // 寄存器值
			sendbuffer[8 + startindex] = strToByte(padLeft(para, 2));// FF,00,80
			CRC16(sendbuffer, 9, 9 + startindex);
			len = 11;
			break;
		case "0405":// 开灯光照度,5-250
		case "0406":// 每路开灯人数,5-50
		case "0407":// 作息个数,1-4
			sendbuffer[4 + startindex] = 0x00; // 寄存器数量
			sendbuffer[5 + startindex] = 0x01;
			sendbuffer[6 + startindex] = 0x02; // 字节数
			sendbuffer[7 + startindex] = 0x00; // 寄存器值
			sendbuffer[8 + startindex] = (byte) (Integer.parseInt(para));
			CRC16(sendbuffer, 9, 9 + startindex);
			len = 11;
			break;
		case "0408":// 作息制度1
		case "0412":// 作息制度2
			sendbuffer[4 + startindex] = 0x00; // 寄存器数量
			sendbuffer[5 + startindex] = 0x0A;
			sendbuffer[6 + startindex] = 0x14; // 字节数
			for (int i = 0; i < 10; i++)
			{
				sendbuffer[7 + i * 2 + startindex] = 0x00;
				sendbuffer[8 + i * 2 + startindex] = strToByte(para.substring(
						i * 2, i * 2 + 2));
			}
			CRC16(sendbuffer, 27, 27 + startindex);
			len = 29;
			break;
		default:
			break;
