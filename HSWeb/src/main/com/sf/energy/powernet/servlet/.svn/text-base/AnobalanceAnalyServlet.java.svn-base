...
import com.sf.energy.powernet.service.AmNobalance;

public class AnobalanceAnalyServlet extends HttpServlet {
	...
	private	void  getResultData(HttpServletRequest request, HttpServletResponse response) throws IOException, SQLException
	{
		...
		if(styleID == 0)
		{
			...
			AmNobalance    anb = new AmNobalance();
			ArrayList<RateNoModel>  list = new ArrayList<RateNoModel>();
			
			for (int i = 0; i < 24; i++)
			{
				rnm = anb.CountAllRateNo(year, month, day, i);	
				list.add(rnm);
			}
			Gson  gson = new Gson();
		...
		else if(styleID ==1)
		{
			...
			for (int i = 0; i < 24; i++)
			{
				rnm = anb.CountRateNoByParent(partID, year, month, day, i);
				list.add(rnm);
			}
			Gson  gson = new Gson();
			String  data = gson.toJson(list);
		...
		else if(styleID ==3)
		{
			...
			for (int i = 0; i < 24; i++)
			{
				...
				rnm = anb.CountRateNo(Ammeter_ID, year, month, day, i);
				list.add(rnm);
			}
			Gson  gson = new Gson();
			String  data = gson.toJson(list);
