...
public class BaojingPrepaymentServlet extends HttpServlet
{
	...
	private void findMethod(HttpServletRequest req, HttpServletResponse resp) throws SQLException, IOException
	{
		...
		if("getDataForBaojingInfoPre".equals(method))
			getDataForBaojingInfoPre(req,resp);
		
		if("deleteBaojingInfoPre".equals(method))
			...
			HttpServletResponse resp) throws IOException, SQLException
	{
		...
		log.writeLog(userLoginName, "预付费告警信息", "删除预付费告警信息");
	 ...
	 * @throws IOException
	 */
	private void getDataForBaojingInfoPre(HttpServletRequest req,
			HttpServletResponse resp) throws SQLException, IOException
	{
		...
		BaoJingInfoForPreDao dao=new BaoJingInfoForPreDao();
		ArrayList<BaoJingInfoModel> list=dao.queryInfo(thePageCount,thePageIndex,selectInfo,startTime,endTime,order);
